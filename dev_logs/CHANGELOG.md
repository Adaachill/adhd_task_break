...
## 2026-06-07: 習慣ごとヒートマップを 7×7 正方形 × 3列レイアウトに
**ブランチ:** claude/heatmap-3col-square-mNqR

### 変更内容
- `src/features/habits/HabitHeatmap.tsx`:
  - `buildGrid(weeks)` を引数化し、集計用（12週）と習慣ごと用（7週）の2種類のグリッドを生成。
  - 習慣ごとのヒートマップを **7週×7日の正方形** に圧縮（`smallCellSize=11`, `smallCellGap=2`）。
  - 習慣カードを `flexWrap` + `width: 33.333%` で **3列グリッド** に並べる。
  - カード内のレイアウトはラベル行（▶ name + ⋯）+ グリッド の縦並びで、画面幅を圧迫しない構成に。
  - 横スクロール ScrollView は撤去（小グリッドはカード幅に収まるため不要）。
  - 集計ヒートマップ（上段）は従来どおり 12週・横スクロール対応。
  - `MonthRow` に `weeks` prop を渡して総幅を可変化。

### 変更意図・背景
習慣が増えると 12週グリッドが縦に積み重なり、空きセルが多いタスクは「とりあえずやってない」感が強調されてしまっていた。直近の継続性が分かれば十分なので、各タスクの草を 7週（約49日）の正方形に圧縮し、3列で並べることで一覧性を大幅に向上。ダッシュボード感覚で全習慣の最近の調子を俯瞰できる。

### 技術的決定事項
- `7×7=49日` の正方形にすることで GitHub の grass よりコンパクトかつタイル的な見た目に。日次の解像度は維持しつつ視線移動を短く。
- セルサイズ 11px は 360px 幅モバイルで 3列に収まる最大値（カード幅 ~120px − padding ≈ 100px、グリッド ~89px）。
- 集計（上段）は逆に「長期トレンドを見たい」要件なので 12週のまま横スクロール。役割を分離。
- selectedDay の状態は引き続きコンポーネント全体で共有。タスクをまたぐ比較に便利。

### 残課題・次のステップ
- ウィンドウ幅に応じた可変列数（モバイル 2列 / タブレット 3列 / デスクトップ 4列）。
- ストリーク数バッジを各カードに追加。
- カードを長押しで操作モーダル直接オープン。
